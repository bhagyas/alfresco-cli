...
import { AlfrescoApi, AuthenticationApi, NodeEntry, NodesApi, PeopleApi, PersonBodyCreate, SearchApi, SitesApi, UploadApi, VersionsApi } from '@alfresco/js-api';
import { ChangesApi } from '@alfresco/js-api/typings/src/api-legacy/content-rest-api/src/api/changesApi';
...
let siteNameAutoCompletion = async (input, callback) => {
  let sitesApi = new SitesApi(alfrescoJsApi);
  let results: string[] = [];
  try {
    let sites = await sitesApi.listSites();
...
let nodeNameAutoCompletion = async (input, callback) => {
  try {
    ...
    let nodesApi = new NodesApi(alfrescoJsApi);
    await nodesApi.listNodeChildren(currentNodeRef).then(
      data => {
        ...
        let resultsHistory: string[] = [];
        try {
          const stored = vorpal.localStorage.getItem('resultsHistory');
          if (stored) {
            resultsHistory = JSON.parse(stored);
          }
        } catch {
          resultsHistory = [];
        }
...
vorpal
  .command('login <host> <username>', 'Login to an Alfresco instance.')
  ...
  .action(function (args, callback) {
    ...
    let loginToAlfresco = (username, password) => {
      return new Promise((resolve, reject) => {
        alfrescoJsApi = new AlfrescoApi({ provider: 'ECM', hostEcm: args.host });
        return alfrescoJsApi.login(username, password)
          .then(function (data) {
            ...
            let error;
            try {
              error = JSON.parse(data.response.text).error.briefSummary;
            } catch {
              error = data.response.text || 'Unknown error';
            }
            reject(error);
...
vorpal.command('change site <siteName>', 'Change/Load into a site.')
  .alias('cd-site')
  .alias('load site <siteName>')
  .autocomplete({ data: siteNameAutoCompletion })
  .action((args, callback) => {
    let sitesApi = new SitesApi(alfrescoJsApi);
    sitesApi.listSites().then(function (data) {
...
vorpal
  ...
  .action(function (args, callback) {
    let self = this;
    new SitesApi(alfrescoJsApi).listSites().then(function (data) {
...
vorpal.command('list people', "Lists all users in system.")
  .action(async function (args, callback) {
    try {
      let self = this;
      let data = await new PeopleApi(alfrescoJsApi).listPeople();
  ...
  .action(function (args, cb) {
    ...
    getNodeRefContext(args.nodeRef).then(nodes => {
      nodes.forEach(nodeRef => {
        let nodesApi = new NodesApi(alfrescoJsApi)
        return nodesApi.listParents(nodeRef, {})
          .then(function (data) {
            const totalItems = data.list?.pagination?.totalItems ?? 0;
            self.log('API called successfully. ' + totalItems + ' parent(s) found.');
    ...
    braces.expand(args.filePath).forEach(filePath => {
      ...
      getNodeRefContext(args.destinationNodeRef).then(nodes => {
        nodes.forEach(destinationNodeRef => {
          new UploadApi(alfrescoJsApi).uploadFile(fileToUpload, args.relativePath, destinationNodeRef, null, { autoRename: args.options.autoRename })
...
vorpal.command('view-metadata [nodeRef] [property]', "Shows metadata for the selected node.")
  ...
  .action(function (args, callback) {
    ...
    getNodeRefContext(args.nodeRef).then(nodes => {
      nodes.forEach(nodeRef => {
        new NodesApi(alfrescoJsApi).getNode(nodeRef, { include: ['properties', 'aspectNames'] }).then(function (data) {
          self.log('name: ' + data.entry.name);
          let rows: any = flatten(data);
...
vorpal.command('move <nodeRef> <destinationNodeRef>', "Moves a node to a destination.")
  .autocomplete({ data: nodeNameAutoCompletion })
  .action(function (args, callback) {
    new NodesApi(alfrescoJsApi).moveNode(args.nodeRef, {
      targetParentId: args.destinationNodeRef
    })
  });
...
vorpal.command('create site <siteId> [title] [description]', "Creates a site (Visibility PUBLIC by default)")
  .action((args, callback) => {
    ...
    new SitesApi(alfrescoJsApi).createSite(siteBody as any, { skipAddToFavorites: false, skipConfiguration: false })
      .then(() => {
...
vorpal.command("create person <userName> <password> [email] [firstName] [lastName]", "Creates a new user.")
  .action((args, callback) => {
    ...
    new PeopleApi(alfrescoJsApi).createPerson(person).then(
      result => {
  ...
  .action((args, callback) => {
    getNodeRefContext(args.nodeRef).then(nodes => {
      nodes.forEach(nodeId => {
        new VersionsApi(alfrescoJsApi).listVersionHistory(nodeId).then(versions => {
          printNodeList(versions.list.entries);
        }, function (error) {
          vorpal.log(error);
        })
      })
...
function getAllAliases(): Array<Alias> {
  try {
    const stored = vorpal.localStorage.getItem('aliases');
    ...
    getNodeRefContext(args.destinationNodeRef).then(nodesList => {
      nodesList.forEach(destinationNodeRef => {
        return braces.expand(args.folderName).forEach(folderName => {
          new NodesApi(alfrescoJsApi).createNode(destinationNodeRef, {
            name: folderName, nodeType: "cm:folder", relativePath: args.path
          }).then(function (data) {
...
let init = async () => {
  ...
  try {
    if (ticket && _host) {
      ...
      await alfrescoJsApi.loginTicket(ticket, null).then(function (data) {
...
vorpal.command('search <query> [language] [alias]', "Searches the repostitory for content.")
  ...
  .action(function (args, callback) {
    ...
    try {
      ...
      new SearchApi(alfrescoJsApi).search({
        "query": {
          "query": query,
  ...
  try {
    entries.forEach(item => {
      ...
      parameters.forEach(param => {
        ...
        if (resultsHistory) {
          try {
            currentHistory = JSON.parse(resultsHistory);
          } catch {
            currentHistory = [];
          }
        }
        currentHistory.push(item.entry[param]);
        vorpal.localStorage.setItem('resultsHistory', JSON.stringify(currentHistory));
      })
    })
  } catch (e) {
    vorpal.localStorage.removeItem('resultsHistory');
...
async function getParent(nodeRef): Promise<string> {
  ...
  return new NodesApi(alfrescoJsApi).listParents(nodeRef)
    .then(function (data) {
  ...
  .action((args, callback) => {
    ...
    if (lastDeleted) {
      ...
      nodesList.forEach(node => {
        throw new Error("No longer supported by the official API.");
...
vorpal.command('delete <nodeRef> [nodeRefPattern] [force]', 'Deletes a nodeRef matching a pattern')
  ...
  .action(function (args, callback) {
    ...
    let deleteNode = (nodeRef, permanent) => {
      ...
      let deleteOp = new NodesApi(alfrescoJsApi).deleteNode(nodeRef, { permanent })
  ...
  .action(function (args, callback) {
    ...
    getNodeRefContext(args.nodeRefPattern).then(list => {
      list.forEach(nodeRef => {
        ...
        new NodesApi(alfrescoJsApi).listNodeChildren(nodeRef).then(function (data) {
...
async function getNodeRefContext(nodeRefPattern: string, explicit = false): Promise<Array<string>> {
  ...
  if (nodeRefPattern) {
    ...
    await new NodesApi(alfrescoJsApi).listNodeChildren(storedNodeRef).then(function (data) {
